...
import com.yacoding.xt.sso.model.enums.LoginType;
...
public class LoginDomain {
    ...
    public static final String secretKey = "yacoding!@#$%xtsso&^#$#@@";
    ...
    public CallResult<Object> wxLoginCallBack() {
        ...
        try {
            ...
            //只有在 可以获取用户登录信息的时候 才能生效
            //refresh_token 无法应用于用户登录 刷新token 加快 访问 只能应用于 可以拿到登录信息的场景
...
//            }
    ...
    public String buildGzhUrl() {
        String url = this.loginDomainRepository.buildGzhUrl();//调用微信登录的信息
        return url ;
    }
...
//                String unionId = wxMpOAuth2AccessToken.getUnionId();
                // 需要保存refreshToken 保存在redis当中，过期时间 设置为 28天
                loginDomainRepository.redisTemplate.opsForValue().set(RedisKey.GZH_REFRESH_TOKEN  + wxMpOAuth2AccessToken.getUnionId(), refreshToken, 28, TimeUnit.DAYS);
            }else{
